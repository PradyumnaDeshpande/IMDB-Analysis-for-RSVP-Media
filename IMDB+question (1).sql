...
SELECT count(*) FROM movie  -- 20 rows are null
WHERE country IS NULL;
SELECT count(*) FROM movie  -- 3724 rows are null
WHERE worlwide_gross_income IS NULL; 
SELECT count(*) FROM movie  -- 194 rows are null
WHERE languages IS NULL;
SELECT count(*) FROM movie  -- 528 rows are null
...
-- It maybe observed that the number of movies released decreases every year.
...
SELECT count(*) as num_movies , year FROM movie
...
SELECT count(movie_id) AS movies_produced , genre
FROM genre 
GROUP BY genre
ORDER BY count(movie_id) desc;

-- Drama is the genre where highest number of movies are produced
...
SELECT  genre, ROUND(AVG(duration),2) AS duration 
from movie AS m 
...
SELECT  title, avg_rating AS avg_rating,
...
-- Highest movies i.e 2257 got 7 rating. Most movies lie between ratings 6 and 8 
...
SELECT genre, count(id) AS movies_produced
FROM movie AS m INNER JOIN
genre AS g
ON g.movie_id= m.id
INNER JOIN ratings as r
...
-- 361 movies having median_rating 8.
...
WITH actor_summary
     AS (SELECT N.NAME AS actor_name, total_votes , Count(r.movie_id) AS movie_count,
...
WITH actress_summary
     AS (SELECT N.NAME AS actress_name, total_votes , Count(r.movie_id) AS movie_count,
...
WITH next_published_summary AS
(
           SELECT     d.name_id,
                      NAME,
                      d.movie_id,
